...
public class CustomLauncher extends Launcher {
	...
	private static JaegerTracer getTracer(String name) {
		SamplerConfiguration samplerConfig = SamplerConfiguration.fromEnv().withType(ProbabilisticSampler.TYPE)
				.withParam(0.001);

		ReporterConfiguration reporterConfig = ReporterConfiguration.fromEnv().withLogSpans(true);

		Configuration config = new Configuration(name).withSampler(samplerConfig).withReporter(reporterConfig);
